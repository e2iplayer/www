...
if availSpaceMB < requiredFreeSpaceMB:
    ...
    if answer != 'Y':
        raise Exception("Not enough disk space for installing exteplayer3 PNG plugin!\nAt least %s MB is required." % requiredFreeSpaceMB)
...
downloadPackage = "exteplayer3png%s.so" % packageConfig
installPackage = "exteplayer3png.so"
...
for cmd in [INSTALL_BASE + 'usr/bin/wget', 'wget', 'fullwget', '/usr/lib/enigma2/python/Plugins/Extensions/IPTVPlayer/bin/wget']:
    try:
        file = os.popen(cmd + ' --no-check-certificate "https://www.e2iplayer.gitlab.io/resources/packages/bin/%s/%s" -O "/tmp/%s" ' % (e2iPlatform, downloadPackage, downloadPackage))
        data = file.read()
        ret = file.close()
        if ret in [0, None]:
            WGET = cmd
...
if WGET == '':
    raise Exception('Download package %s failed!' % downloadPackage)
...
if answer == 'Y':
    installPath = '%s/lib/%s' % (INSTALL_BASE, installPackage)
    ret = os.system("mkdir -p %s/lib && cp /tmp/%s %s && chmod 777 %s " % (INSTALL_BASE, downloadPackage, installPath, installPath))

os.system('rm -f /tmp/%s' % downloadPackage)

if ret not in [None, 0]:
    raise Exception('%s installation failed with return code: %s' % (installPackage, ret))
...
if answer == 'Y':
    if HasPackage():
        ...
        raise Exception('Installed %s is NOT working correctly!' % installPackage)

# cd /tmp && rm -f e3png.py && wget https://www.e2iplayer.gitlab.io/e3png.py && python e3png.py
