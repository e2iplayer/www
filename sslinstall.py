...
opensslPackageConfig = '%s_%s%s' % (e2iPlatform, installOld, installFPU)
opensslInstallPackage = 'openssl_%s.tar.gz' % opensslPackageConfig
if opensslPackageConfig not in ['armv5t_fpu_soft',
                               'armv5t_fpu_soft', 
                               'armv5t_fpu_softfp',
                               'armv5t_fpu_softfp',
                               'armv7_fpu_hard',
                               ...
                               'sh4_old_fpu_hard',
                               'sh4_old_fpu_hard']:
    raise Exception('At now there is no\n"%s"\npackage available!\nYou can request it via e-mail: e2iplayer@yahoo.com' % opensslInstallPackage)

printDBG("Slected openssl package: %s" % opensslInstallPackage)
...
if WGET == '':
    raise Exception('Download package %s failed!' % opensslInstallPackage)

msg = 'Package %s ready to install.\nDo you want to proceed?' % opensslInstallPackage
answer = ''
while answer not in ['Y', 'N']:
    answer = raw_input(MSG_FORMAT.format(msg) + "\nY/N: ").strip().upper()
    msg = ''
...
if answer == 'Y':
    ret = os.system("mkdir -p %s && tar -xvf /tmp/%s -C %s " % (INSTALL_BASE, opensslInstallPackage, INSTALL_BASE))

os.system('rm -f /tmp/%s' % opensslInstallPackage)

if ret not in [None, 0]:
    raise Exception('Openssl installation failed with return code: %s' % (ret))
